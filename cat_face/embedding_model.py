...
class EmbeddingExtractor:
    """Extract embeddings using a lightweight MobileNet backbone."""
...
class EmbeddingModel:
    ...
    def load(cls, path: Path | str) -> "EmbeddingModel":
        ...
        return cls(centroids=centroids, label_ids=label_ids)

    def save(self, path: Path | str) -> None:
        path = Path(path)
        path.parent.mkdir(parents=True, exist_ok=True)
        np.savez(path, centroids=self.centroids, label_ids=self.label_ids)
