...
| `recorder` | `output_dir`, `min_duration`, `max_duration`, `cooldown`, `absence_grace`, `fps`, `codec`, `picamera_bitrate` | Default settings for `record_cat_video.py` (clip destination, duration bounds, cooldown, absence grace period, encoder options, and Picamera2 bitrate). |
