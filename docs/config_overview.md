...
| `streaming` | `publish_url`, `publish_format`, `publish_options`, `bitrate`, `public_url` | Optional hardware streaming publisher. Point `publish_url` at something like `rtsp://127.0.0.1:8554/cam` (MediaMTX), set `public_url` to whatever browser endpoint should be embedded (e.g., `http://pi-host:8889/cam`). |
