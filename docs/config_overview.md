...
| `recorder` | `output_dir`, `min_duration`, `max_duration`, `cooldown`, `absence_grace`, `fps`, `codec`, `picamera_bitrate`, `prerecord_ms` | Default settings for `record_cat_video.py` (clip destination, duration bounds, cooldown, absence grace period, encoder options, Picamera2 bitrate, and pre-roll buffer length). |
| `clip_processing` | `clips_dir`, `enable_compression`, `save_limit`, `training_refresh_count`, `recognition_margin`, `compression_crf`, `watch_interval`, `detection_interval`, `trim_padding_seconds` | Controls how `process_clips.py` and the clip watcher behave (clip source directory, optional compression toggle, per-clip sampling limit, how many frames to promote to training, recognition margin for auto-tagging, H.265 CRF, watcher poll interval, YOLO cadence when scanning clips, and how much padding to keep when trimming clips down to the cat’s appearance). |
| `streaming` | `publish_url`, `publish_format`, `publish_options`, `bitrate`, `public_url`, `viewer_*` | Optional hardware streaming publisher. Point `publish_url` at something like `rtsp://127.0.0.1:8554/cam` (MediaMTX). If you omit `public_url`, the web UI derives `viewer_protocol://<host>:viewer_port/viewer_path` automatically (defaults to `http://pi-host:8889/<stream-name>`). |

Update `vision.face_size` once to keep recording, training, and recognition aligned. Point `detection.model` at the YOLO ONNX file you want to run (for example, a 640×640 `yolov5n` export) and adjust `input_size`, confidence, or providers to match your hardware. Train embeddings via `python train_embeddings.py`, then set `recognition.embedding_threshold` to a value that balances precision/recall for your cats. All clip collection now flows through `record_cat_video.py`, which triggers on motion first and relies on clip processing to confirm cats, so there’s no separate still-image capture step. Picamera2 is now mandatory for the recorder; once installed, it uses the hardware encoder both for clips and for the optional RTSP/HLS publisher (feed it into MediaMTX or any RTSP relay) while motion detection continues to run off the low-resolution stream.
...
- `picamera_bitrate`: Target bitrate (bits/sec) for Picamera2’s native H.264 encoder. Raise it for higher quality clips; lower it to reduce file sizes or ease SD-card writes.
- `prerecord_ms`: Milliseconds of footage retained in the circular buffer before a clip officially starts. Set to `0` to disable pre-roll; otherwise, completed clips include this much footage leading into the trigger event.
